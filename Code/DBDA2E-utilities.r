...
diagMCMC = function( codaObject , parName=varnames(codaObject)[1] ,
  ...
  fileName=paste0(saveName,"Diag",parName)
  if(length(grep(".pdf",fileName))==0){fileName=sprintf("%s.pdf",fileName)}	
  print(fileName)
...
diagStanFit = function( stanFit , parName ,
  ...
  DBDAplColors = c("skyblue","black","royalblue","steelblue")
  #openGraph(height=5,width=7)
  pdf(file=paste0(saveName,"Diag",parName),height=5,width=7)
